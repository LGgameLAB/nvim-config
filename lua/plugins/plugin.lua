	...
	},
	{
		...
		config = function()
			require("mason-lspconfig").setup({
				...
				-- mason-lspconfig >= 2.0 auto-enables installed servers via
				-- vim.lsp.enable() by default (replaces automatic_installation).
			})
		end,
	},
	{
		...
		config = function()
			-- Neovim >= 0.11: server setup now goes through vim.lsp.config()/
			-- vim.lsp.enable() instead of require('lspconfig').<server>.setup({}).
			...
			-- Enable LSP-based autocompletion for every server
			...
			-- mason-lspconfig already auto-enables installed servers, but
			-- enabling explicitly here is harmless and keeps things working
			-- even if load order or automatic_enable ever changes.
